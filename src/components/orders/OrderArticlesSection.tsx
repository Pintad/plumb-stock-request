...
const OrderArticlesSection = ({ articles, isAdmin, onItemCompletionToggle, isMobile }: OrderArticlesSectionProps) => {
  ...
  return (
    <div className="border-t pt-4">
      <p className={`font-medium mb-2 ${isMobile ? 'text-sm' : ''}`}>Articles</p>
      {isAdmin ? (
        <div className="space-y-2">
          {articles.map((article, index) => (
            <div key={index} className={`flex items-center justify-between py-2 ${isMobile ? 'px-2' : 'px-3'} bg-gray-50 rounded-md`}>
              <div className="flex items-center gap-3 flex-1">
                ...
                >
                  <div className={`font-medium ${isMobile ? 'text-sm' : ''}`}>{formatArticleDisplay(article)}</div>
              ...
              <div className="flex items-center gap-3">
                <span className={`font-bold ${isMobile ? 'text-base' : 'text-lg'} px-3 py-1 bg-gray-100 rounded-md`}>
                  {article.quantity} <span className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-600 ml-1`}>{getArticleUnit(article)}</span>
                </span>
                ...
                <Popover>
                  ...
                  <PopoverContent className={`${isMobile ? 'w-64' : 'w-80'} p-0`}>
                    <div className="p-4">
                      {article.imageUrl ? (
                        <img 
